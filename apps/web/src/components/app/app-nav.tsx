...
import { CalendarDays, ChartNoAxesCombined, CircleUserRound, FileText, House, Leaf, LogOut, PlusCircle, ShieldCheck, Syringe, UtensilsCrossed, WalletCards } from 'lucide-react';
...
const sidebarItems = [
  ...
  { href: '/app/alimentacao', label: 'Alimentação', icon: UtensilsCrossed },
  { href: '/app/aplicacao', label: 'Aplicação', icon: Syringe },
...
export function AppNav({ name }: { name: string }) {
  ...
  return <>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 flex h-[calc(4rem+env(safe-area-inset-bottom))] items-start justify-around border-t bg-card px-2 pt-1 pb-[env(safe-area-inset-bottom)] md:hidden">{mobileNavItems.map(({ href, label, icon: Icon }) => { const active = href === '/app' ? pathname === href : pathname.startsWith(href); return <Link key={href} href={href} className={cn('flex min-w-0 flex-1 flex-col items-center gap-0.5 pt-1 text-[11px] font-medium text-muted-foreground transition-colors', active && 'text-primary')}><Icon className="size-6" /><span className="max-w-full truncate leading-tight">{label}</span></Link>; })}</nav>
  </>;
}