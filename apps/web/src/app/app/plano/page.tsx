...
export default function PlansPage() {
  ...
  async function subscribe(planCode: PlanDefinition['code']) {
    setSubmitting(planCode); setError('');
    try {
      ...
      const body = await response.json() as { checkoutUrl?: string | null; error?: string };
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível assinar.'); }
  ...
  return <div className="space-y-7">
    ...
    <p className="text-center text-xs text-muted-foreground">Pagamento seguro processado pela Efí. Você pode cancelar quando quiser.</p>
  </div>;
}
